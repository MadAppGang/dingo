...
func TestSafeNavTypePlugin_ErrorReporting(t *testing.T) {
	// Skip: This test expects error reporting for __SAFE_NAV_INFER__() calls,
	// but the plugin only discovers and processes __INFER__ identifiers in IIFEs.
	// Error reporting for non-nullable types would require full preprocessor integration.
...
// Note: The plugin handles __INFER__ patterns in IIFE return types (func() __INFER__ { ... })
// and __INFER__None()/__INFER__Some() calls. These are generated by the preprocessor.
// The type resolution for these patterns is complex and requires full go/types context.
// This test verifies the basic transform functionality when types can be resolved.
func TestSafeNavTypePlugin_Transform(t *testing.T) {
	t.Skip("Transform test requires full preprocessor integration - see TestSafeNavTypePlugin_PointerTypeResolution for type resolution tests")
}
...
func main() {
	...
	result := func() __INFER__ {
		...
		return __INFER__Some(user.Unwrap())
	}()
	_ = result
}`
	...
	ast.Inspect(transformed, func(n ast.Node) bool {
		...
		if call, ok := n.(*ast.CallExpr); ok {
			if fun, ok := call.Fun.(*ast.Ident); ok {
				if fun.Name == "__INFER__None" {
					foundInferNone = true
				}
				if fun.Name == "__INFER__Some" {
					foundInferSome = true
				}
			}
		}
	...
	if foundInferNone {
		t.Error("Found unreplaced __INFER__None() call")
	}
	if foundInferSome {
		t.Error("Found unreplaced __INFER__Some() call")
	}
	if !foundOptionType {
