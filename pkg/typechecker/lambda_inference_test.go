...
func TestLambdaInference_DgoDependentRange(t *testing.T) {
	src := `package main
...
func main() {
	...
	byCategory := dgo.GroupBy(products, func(p any) any { return p.Category })
	for _, prods := range byCategory {
		names := dgo.Map(prods, func(p any) any { return p.Name })
		_ = names
	}
}
`
	...
	for pass := 0; pass < 5; pass++ {
		...
		checker, err = New(fset, file, "main")
		if err != nil {
			t.Fatalf("type checker pass %d: %v", pass+1, err)
		}
	}

	var generated bytes.Buffer
	if err := printer.Fprint(&generated, fset, file); err != nil {
		t.Fatalf("print transformed source: %v", err)
	}
	if got := strings.Count(generated.String(), "func(p Product) string"); got != 2 {
		t.Fatalf("expected both lambdas to infer func(p Product) string, got %d:\n%s", got, generated.String())
	}
}
